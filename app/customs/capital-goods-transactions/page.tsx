...
export default function CapitalGoodsTransactionsPage() {
  ...
  const [selectedTransaction, setSelectedTransaction] = useState<CapitalGoodsTransaction | null>(null);
  ...
  const handleEdit = (transaction: CapitalGoodsTransaction) => {
    setSelectedTransaction(transaction);
    setEditDialogOpen(true);
  };

  const handleDeleteClick = (transaction: CapitalGoodsTransaction) => {
    setSelectedTransaction(transaction);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setDeleting(true);
    try {
      const response = await fetch(`/api/customs/capital-goods-transactions/${selectedTransaction.id}`, {
        method: 'DELETE',
      });
  ...
  const handleExportExcel = () => {
    const exportData = filteredData.map((row, index) => ({
      ...
      'Transaction Date': formatDate(row.transactionDate),
  ...
  return (
    ...
    >
      ...
      <EditCapitalGoodsTransactionDialog
        open={editDialogOpen}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete this capital goods transaction?
            {selectedTransaction && (
              <>
                <br /><br />
