...
export default function BeginningCapitalGoodsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/customs/beginning-capital-goods${params.toString() ? `?${params.toString()}` : ''}`
      );
      if (!response.ok) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/customs/beginning-capital-goods/${selectedItem.id}`, {
        method: 'DELETE',
      });
  ...
  const handleFormSubmit = async (formData: BeginningStockFormData) => {
    try {
      const endpoint = formMode === 'add'
        ? '/api/customs/beginning-capital-goods'
        : `/api/customs/beginning-capital-goods/${selectedItem?.id}`;
  ...
  const handleImportSubmit = async (records: any[]) => {
    try {
      const response = await fetch('/api/customs/beginning-capital-goods/import', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <ReportLayout
      title="Beginning Stock - Capital Goods"
      subtitle="Manage beginning balance data for capital goods mutations"
      actions={
