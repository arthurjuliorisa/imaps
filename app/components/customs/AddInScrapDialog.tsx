...
interface AddInScrapDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function AddInScrapDialog({
  open,
  onClose,
  onSuccess,
}: AddInScrapDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/customs/scrap-transactions/in', {
        method: 'POST',
        headers: {
          ...
          <Stack spacing={3}>
            ...
            >
              ...
              <Typography variant="body2" color="text.secondary">
                This transaction will record the incoming quantity of scrap. The system will automatically update the stock balance.
              </Typography>
            </Box>
          </Stack>
