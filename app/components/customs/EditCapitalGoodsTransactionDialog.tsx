...
interface EditCapitalGoodsTransactionDialogProps {
  open: boolean;
  transaction: CapitalGoodsTransaction | null;
  onClose: () => void;
  onSuccess: () => void;
}
...
export function EditCapitalGoodsTransactionDialog({
  ...
  onClose,
  onSuccess,
}: EditCapitalGoodsTransactionDialogProps) {
  ...
  /**
   * Pre-check stock availability when qty changes
   */
  const checkStockAvailabilityFn = async () => {
    if (!transaction || !formData.qty || formData.qty <= 0) {
      setStockCheckResult(null);
      return;
    }
          ...
          items: [
            {
              ...
              itemType: transaction.transactionType,
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/customs/capital-goods-transactions/${transaction.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
        <DialogTitle>
          Edit Capital Goods Transaction
        </DialogTitle>
        <DialogContent>
          <Alert severity="info" sx={{ mb: 2 }}>
            This transaction is from the outgoing goods record. Changes will update the outgoing goods data.
          </Alert>
