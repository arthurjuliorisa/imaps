...
/**
 * GET /api/customs/raw-material/template
 * Generates and downloads an Excel template file for raw material mutation imports
 *
 * Returns:
...
export async function GET() {
  try {
    ...
    // ============================================
    // SHEET 1: Raw Material Import Template (Main Sheet)
    // ============================================

    const worksheet = workbook.addWorksheet('Raw Material Import Template');

    // Define headers
    const headers = ['Date', 'Item Code', 'Incoming', 'Remarks'];

    // Define format hints (row 2)
    const formatHints = ['YYYY-MM-DD', 'e.g., RM-001', 'Positive number', 'Optional notes'];

    // Define sample data (rows 3-5)
    const sampleData = [
      [new Date('2025-01-15'), 'RM-001', 50, 'New scrap received'],
      [new Date('2025-01-16'), 'RM-002', 75, 'Additional scrap'],
      [new Date('2025-01-17'), 'RM-001', 30, 'Daily intake'],
    ];
    ...
    const instructionsData = [
      ['Raw Material Mutation Import Template - Instructions'],
      [],
      ['How to Use This Template:'],
      ['1. Fill in your raw material mutation data starting from Row 3 (after the format hints)'],
      ...
      [],
      ['Item Code:'],
      ['  - The unique code of the scrap item (e.g., RM-001)'],
      ['  - Required field'],
      ...
      [],
      ['Remarks:'],
      ['  - Optional notes about the raw material mutation'],
      ...
      ['2025-01-15   | RM-001   | 50       | New scrap received'],
      ['2025-01-16   | RM-002   | 75.50    | Additional scrap from production'],
      ['2025-01-17   | RM-001   | 30       | Daily intake'],
      [],
    ...
    const filename = `Raw Material_Import_Template_${timestamp}.xlsx`;
