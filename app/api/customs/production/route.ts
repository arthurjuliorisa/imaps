// @ts-nocheck
// TODO: This file needs to be rewritten - productionMutation model doesn't exist
...
/**
 * GET /api/customs/production
 * Fetch production mutations with date range filtering
...
export async function GET(request: Request) {
  try {
    ...
    const productionMutations = await prisma.productionMutation.findMany({
      where,
      include: {
        Item: {
          select: {
    ...
    const transformedData = productionMutations.map((mutation) => ({
      ...mutation,
...
/**
 * POST /api/customs/production
 * Create a single INCOMING-only production mutation entry
...
export async function POST(request: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const previousRecord = await tx.productionMutation.findFirst({
        where: {
          itemId,
      ...
      const id = `PM-${normalizedDate.getTime()}-${itemId}`;
      const productionMutation = await tx.productionMutation.create({
        data: {
          id,
          date: normalizedDate,
          itemId,
      ...
      const subsequentRecords = await tx.productionMutation.findMany({
        where: {
          itemId,
      ...
      let runningEnding = productionMutation.ending;
    ...
    console.error('[API Error] Failed to create production mutation:', error);

    // Bug #13: Enhanced error handling for Prisma errors
    if (error.code === 'P2002') {
      return NextResponse.json(
        { message: 'A production mutation for this item and date already exists' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: 'Error creating production mutation' },
      { status: 500 }
    );
  }
}
