...
/**
 * POST /api/customs/production/import
 * Import INCOMING-only production mutations from Excel
...
export async function POST(request: Request) {
  try {
    ...
    try {
      const results = await prisma.$transaction(async (tx) => {
        ...
        for (const record of sortedRecords) {
          ...
          const result = await tx.productionMutation.upsert({
            where: {
              date_itemId: {
                date: record.date,
                itemId: record.itemId,
            ...
            create: {
              id: `PM-${record.date.getTime()}-${record.itemId}`,
              date: record.date,
              itemId: record.itemId,
              uomId: record.uomId,
              beginning,
    ...
    console.error('[API Error] Failed to import production mutations:', error);
    ...
    return NextResponse.json(
      { message: 'Error importing production mutations' },
      { status: 500 }
    );
  }
}
