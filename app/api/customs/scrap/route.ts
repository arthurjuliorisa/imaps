...
/**
 * GET /api/customs/scrap
 * Fetch scrap mutations with date range filtering
...
export async function POST(request: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      let runningEnding = scrapMutation.ending;
    ...
    console.error('[API Error] Failed to create scrap mutation:', error);
    ...
    return NextResponse.json(
      { message: 'Error creating scrap mutation' },
      { status: 500 }
    );
  }
}
