...
/**
 * POST /api/customs/capital-goods/import
 * Import INCOMING-only capital goods mutations from Excel
...
export async function POST(request: Request) {
  try {
    ...
    try {
      const results = await prisma.$transaction(async (tx) => {
        ...
        for (const record of sortedRecords) {
          ...
          const result = await tx.capitalGoodsMutation.upsert({
            where: {
              date_itemId: {
                date: record.date,
                itemId: record.itemId,
            ...
            create: {
              id: `CGM-${record.date.getTime()}-${record.itemId}`,
              date: record.date,
              itemId: record.itemId,
              uomId: record.uomId,
              beginning,
    ...
    console.error('[API Error] Failed to import capital goods mutations:', error);
    ...
    return NextResponse.json(
      { message: 'Error importing capital goods mutations' },
      { status: 500 }
    );
  }
}
