 ...
 * Fetch capital goods mutations with date range filtering
...
/**
 * POST /api/customs/capital-goods
 * Create a single INCOMING-only capital goods mutation entry
...
export async function POST(request: Request) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const previousRecord = await tx.capitalGoodsMutation.findFirst({
        where: {
          itemId,
      ...
      const subsequentRecords = await tx.capitalGoodsMutation.findMany({
        where: {
          itemId,
      ...
      let runningEnding = capitalGoodsMutation.ending;
    ...
    console.error('[API Error] Failed to create capital goods mutation:', error);

    // Bug #13: Enhanced error handling for Prisma errors
    if (error.code === 'P2002') {
      return NextResponse.json(
        { message: 'A capital goods mutation for this item and date already exists' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { message: 'Error creating capital goods mutation' },
      { status: 500 }
    );
  }
}
