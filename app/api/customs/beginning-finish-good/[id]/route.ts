...
async function recalculateMutationRecords(
  ...
  const firstMutation = await tx.productionMutation.findFirst({
    where: {
      itemId,
      date: { gte: fromDate },
    },
  ...
  const subsequentMutations = await tx.productionMutation.findMany({
    where: {
      itemId,
...
/**
 * GET /api/customs/beginning-finish-good/[id]
...
/**
 * PUT /api/customs/beginning-finish-good/[id]
...
/**
 * DELETE /api/customs/beginning-finish-good/[id]
