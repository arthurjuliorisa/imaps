 ...
 * Generates and downloads an Excel template file for beginning stock imports (Finished Goods)
 *
 * Returns:
...
export async function GET() {
  try {
    ...
    const formatHints = ['e.g., FG-001', 'Informational only', 'e.g., KG', 'Positive number > 0', 'DD/MM/YYYY', 'Optional notes'];

    // Define sample data (rows 3-5)
    const sampleData = [
      ['FG-001', 'Finished Good A', 'KG', 100, '01/01/2025', 'Opening balance'],
      ['FG-002', 'Finished Good B', 'PCS', 250.5, '01/01/2025', 'Initial stock'],
      ['FG-003', 'Finished Good C', 'BOX', 50, '15/01/2025', 'Beginning inventory'],
    ];
    ...
    const instructionsData = [
      ['Beginning Stock (Finished Good) Import Template - Instructions'],
      [],
      ...
      [],
      ['Item Code* (REQUIRED):'],
      ['  - The unique code of the finished good item (e.g., FG-001)'],
      ['  - Must exist in the Item master data with type "Finished Good"'],
      ['  - Case sensitive'],
      [],
      ...
      ['  - Item Code must exist in the system and be of type "Finished Good"'],
    ...
    const filename = `Beginning_Stock_Finished_Good_Template_${timestamp}.xlsx`;
