...
export default function RawMaterialMutationPage() {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/customs/raw-material/export?${params}`);
      if (!response.ok) {
        throw new Error(await readExportErrorMessage(response));
      }

      await downloadExcelResponse(response, `Laporan_Bahan_Baku_${startDate}_${endDate}.xlsx`);
      toast.success('Excel export downloaded.');
    } catch (error) {
      console.error('Error exporting raw material Excel:', error);
