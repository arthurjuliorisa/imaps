...
interface AdjustmentTransactionDetail {
...
export default function AdjustmentDetailPage() {
  ...
  const [data, setData] = useState<AdjustmentTransactionDetail | null>(null);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const url = `/api/customs/stock-count/adjustment/${id}${queryString ? `?${queryString}` : ''}`;
    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      >
        ...
        <Link
          ...
          onClick={() => router.push('/customs/stock-count/adjustment')}
        >
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Box>
            ...
            <Typography variant="body1" color="text.secondary">
              View complete information for adjustment transaction
            </Typography>
          </Box>
          <Button
            variant="outlined"
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ bgcolor: alpha(theme.palette.primary.main, 0.08) }}>
                ...
                <TableCell sx={{ fontWeight: 600 }} align="right">Before Qty</TableCell>
                <TableCell sx={{ fontWeight: 600 }} align="right">After Qty</TableCell>
                <TableCell sx={{ fontWeight: 600 }} align="right">Adjustment Qty</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {data.details.map((detail, index) => (
                <TableRow key={detail.id} hover>
                  ...
                  <TableCell align="right">
                    <Typography variant="body2" fontWeight={600}>
                      {formatQuantity(detail.beforeQty)}
                    </Typography>
                  </TableCell>
                  <TableCell align="right">
                    <Typography variant="body2" fontWeight={600}>
                      {formatQuantity(detail.afterQty)}
                    </Typography>
                  </TableCell>
                  <TableCell align="right">
                    <Typography
                      variant="body2"
                      fontWeight={600}
                      color={detail.adjustmentQty > 0 ? 'success.main' : detail.adjustmentQty < 0 ? 'error.main' : 'text.primary'}
                    >
                      {formatQuantity(detail.adjustmentQty)}
                    </Typography>
                  </TableCell>
                </TableRow>
              ))}
        ...
        <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end' }}>
          <Box sx={{ minWidth: 300 }}>
            ...
            <Stack direction="row" justifyContent="space-between" sx={{ mb: 1 }}>
              ...
              <Typography variant="body1" fontWeight={600}>
                {formatQuantity(data.details.reduce((sum, d) => sum + d.beforeQty, 0))}
              </Typography>
            </Stack>
            <Stack direction="row" justifyContent="space-between">
              ...
              <Typography variant="body1" fontWeight={600}>
                {formatQuantity(data.details.reduce((sum, d) => sum + d.afterQty, 0))}
              </Typography>
            </Stack>
          </Box>
        </Box>
